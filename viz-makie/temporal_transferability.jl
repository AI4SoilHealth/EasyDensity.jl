...
ds = Parquet2.Dataset("eval/all_tc_with.lc_$(version).pq")
    ...
    for (k, t) in enumerate(targets)
        ...
        cb = Colorbar(fig[1, 4], plt;
            label = rich(rich("Count", font=:bold), rich(
                "\n\n Accuracy of\n $(labels[k]) predictions for\n other survey years using models\n trained on 2018 data."),
        ...
        hidespines!.(axs, :t, :r)
        fig
        save(joinpath(@__DIR__, "../figures/temporal_transferability_$(t).pdf"), fig)
    end
end